...
class Network(Base):
  def __init__(self, exp_config):
      ...
      self.data_throughput, self.data_latency = self.calcThroughput(topology, 'data')
      ...
      self.layer_throughput, self.layer_latency = self.calcThroughput(topology, 'layer')
      ...
      self.kernel_throughput, self.kernel_latency = self.calcThroughput(topology, 'kernel')

  def calcThroughput(self, topology, sch_parallelism):
      if 'data' in sch_parallelism:
          throughput, latency = topology.getDataThroughput(self.intra_network.throughput,
                                                           self.inter_network.throughput,
                                                           self.intra_network_latency,
                                                           self.inter_network_latency)
      else if 'kernel' in sch_parallelism:
          throughput, latency = topology.getKernelThroughput(self.intra_network.throughput,
                                                             self.inter_network.throughput,
                                                             self.intra_network_latency,
                                                             self.inter_network_latency)
      else if 'layer' in sch_parallelism:
          throughput, latency = topology.getLayerThroughput(self.intra_network.throughput,
                                                            self.inter_network.throughput,
                                                            self.intra_network_latency,
                                                            self.inter_network_latency)
      else:
...
class SubNetwork(Base):
  ...
  def calcThroughput(self):
      if self.inter and self.topology.interNodeDegree == 0:
        throughput                      = 0
      else if self.intra and self.topology.intraNodeDegree == 0:
        throughput                      = 0
      else:
        ...
        self.num_links                  = min(self.tot_area/self.nominal_area_per_link, perimeter_fraction * perimeter / self.num_links_per_mm)
