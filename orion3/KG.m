...
[dmodel, perf] = dacefit(X, log10(Y), @regpoly2, @corrgauss, theta);


area_q = predictor(new_q, dmodel);
...
[int_dmodel, perf] = dacefit(X, log10(int_pow), @regpoly2, @correxp, theta, lob, upb);
[sw_dmodel, perf] = dacefit(X, log10(sw_pow), @regpoly2, @correxp, theta, lob, upb );
[leak_dmodel, perf] = dacefit(X, log10(leak_pow), @regpoly2, @correxp, theta, lob, upb);

int_powq = predictor(new_q, int_dmodel);
sw_powq = predictor(new_q, sw_dmodel);
leak_powq = predictor(new_q, leak_dmodel);
