...
def sweep_DRAM(base_config, exp_dir, N, kp1, kp2, kp_type, dp, lp):
  ...
  capacity_per_stack = [i * 1024 * 1024 * 1024 for i in [4, 8, 16, 24, 32]]
  ...
  for i, node in enumerate(tech_nodes):
      ...
      config_dict['tech_param']['DRAM']['nominal_frequency']      = freq[i]
      config_dict['tech_param']['DRAM']['area_per_stack']         = area_per_stack[i]
      config_dict['tech_param']['DRAM']['stack_capacity']         = capacity_per_stack[i]
